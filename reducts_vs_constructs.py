...
def predict_class(x_train, x_train_labels, y_test):
    ...
    classifier_name = type(_classifier_).__name__
...
def read_dict_from_csv(csv_file):
    data_dict = {}
    with open(csv_file, mode='r') as _file_:
        csv_reader = csv.reader(_file_)
        next(csv_reader)
...
def run_example_noise_data(base_dir='./resources/train_test_datasets/'):
    for noise_level in [0.01, 0.02, 0.03]:  # change this to variate the level of noise
        ...
        _dict_info_ = run_report_for_all_files(noise_datasets_path, 'results_noise_'+str(noise_level)+'.csv', shortest_attr_subset_dir)
        save_dict(_dict_info_, 'results_noise_'+str(noise_level)+'.json')
        _p_value_ = prob_test(_dict_info_)

        dir_statics = os.path.join("./resources/", classifier_name)
        if not os.path.exists(dir_statics):
            # Create the directory if it does not exist
            print(f'The directory \'{dir_statics}\' does not exist, creating the directory...')
            os.makedirs(dir_statics)

        with open(os.path.join(dir_statics, "statist_"+str(noise_level)+'.txt'), "w") as _file_:
            # Write the string to the file
            _file_.write('p_value = '+str(_p_value_))


def run_example_single_dataset():
    _base_dir_ = './resources/train_test_datasets/'
    reduct_constr_dir = './resources/shortest_red_constr/'
    base_file_name = 'Mushroom-no-missing-values'
    results_dict = {}
    run_model(_base_dir_, base_file_name, results_dict, reduct_constr_dir)
    print(results_dict)
